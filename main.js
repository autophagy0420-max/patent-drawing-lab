...
function finalEditor(){
  ...
  <div class="review-decision-box"><h3>AI 생성 결과에 대한 나의 최종 판단 <span class="badge">필수</span></h3><p class="small muted">문장을 억지로 바꾸는 것이 목적이 아닙니다. 자신의 발명과 비교하여 수정 필요 여부를 판단하고 그 근거를 설명하세요.</p>
    ...
    <div class="core-proof-box"><h3>내 발명의 핵심을 명세서에서 확인하기 <span class="badge">필수</span></h3><p class="small muted">내 발명의 핵심 특징을 한 가지 고르고, 명세서의 어느 부분에 어떻게 표현되어 있는지 직접 설명하세요.</p>
      <label class="field">내 발명의 핵심 특징<textarea id="coreFeature" placeholder="예: 분리막을 슬라이드 방식으로 탈부착할 수 있는 구조">${esc(state.coreFeature||'')}</textarea></label>
      <label class="field">명세서에서 확인한 위치<textarea id="coreFeatureLocation" placeholder="예: 【과제의 해결수단】, 【청구항 1】">${esc(state.coreFeatureLocation||'')}</textarea></label>
      <label class="field">그 부분에 어떻게 표현되어 있나요?<textarea id="coreFeatureExplanation" placeholder="예: 컵 몸통의 슬라이드 홈에 분리막이 위에서 아래로 결합된다고 표현되어 있다.">${esc(state.coreFeatureExplanation||'')}</textarea></label>
    </div>
  ...
  const save=()=>{
    ...
    state.coreFeature=document.querySelector('#coreFeature')?.value.trim()||'';
    state.coreFeatureLocation=document.querySelector('#coreFeatureLocation')?.value.trim()||'';
    state.coreFeatureExplanation=document.querySelector('#coreFeatureExplanation')?.value.trim()||'';
  ...
  document.querySelector('#drawing').onclick=async e=>{
    ...
    if(!state.coreFeature){alert('내 발명의 핵심 특징을 작성하세요.');return}
    if(!state.coreFeatureLocation){alert('핵심 특징이 명세서의 어느 부분에 표현되어 있는지 작성하세요.');return}
    if(!state.coreFeatureExplanation){alert('명세서에 핵심 특징이 어떻게 표현되어 있는지 설명하세요.');return}
...
function done(){
  ...
  document.querySelector('#restart').onclick=()=>{
    ...
    Object.assign(state,{step:0,apiKey:k,title:'',rough:'',sketchDataUrl:'',sketchBase64:'',sketchMime:'',sketchName:'',sketchAnalysis:null,conversation:[],questions:[],answers:{},answerSources:{},interactionLog:[],initialInput:null,brief:null,draft:null,aiInitialDraft:null,aiInitialDraftText:'',finalText:'',revisionNotes:[{change:'',reason:''},{change:'',reason:''}],revisionReasons:{},reviewDecision:'',reviewReason:'',coreFeature:'',coreFeatureLocation:'',coreFeatureExplanation:'',drawingPrompt:''});
    render();
  };
}
